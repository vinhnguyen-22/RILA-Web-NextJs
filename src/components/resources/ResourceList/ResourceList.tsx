...
export const ResourceList: FC<ResourceList> = ({ data }) => {
  ...
  const toggleCategory = (cat: ResourceType) => {
    if (!selectedCategories.includes(cat)) return setSelectedCategories((v) => [...v, cat]);
        ...
        {Object.entries(RESOURCE_ICONS).map(([k, Icon]) => (
          <button
            ...
            className={classes('rounded-lg p-2', {
              'bg-gray-200 dark:bg-gray-900 shadow dark:shadow-gray-600': selectedCategories.includes(
                k as ResourceType
              ),
              'bg-gray-100 dark:bg-gray-700': !selectedCategories.includes(k as ResourceType),
            })}
          >
